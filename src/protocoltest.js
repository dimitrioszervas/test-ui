...
//import { calculateReedSolomonShards } from "./middleware/ReedSolomon";
//import { calculateNumberOfShardsPerServer } from "./middleware/ReedSolomon";
import { encrypt } from "./utils/CryptoUtilsnew";
import { calculateHMAC } from "./utils/CryptoUtilsnew";
...
function base64ToArrayBuffer(base64) {
  try {
      ...
      console.error("Failed to convert base64 to array buffer:", error);
      // Optionally, handle the error by returning null or an empty buffer
  ...
  try {
    ...
    */
    ...
    await axios
      ...
      CBOR = await cbor.decode(base64ToArrayBuffer(responseData));      
...
{/*import axios from "axios";
import cbor from "cbor-js";
import {calculateHMAC, importAesGcmKey, importHmacKey, encrypt} from "./utils/CryptoUtilsnew";
import {calculateReedSolomonShards, calculateNumberOfShardsPerServer} from './middleware/ReedSolomon'
...
export const encryptDataAndSendtoServer = async (ENCRYPTS, SIGNS, SRC, endpoint, numServers, transactionData, signKeys) => {
  try {

    console.log("deviceENCRYPTS: ", ENCRYPTS);
    console.log("deviceSIGNS: ", SIGNS);
    // Initial CBOR encoding of transaction data
    let cborData = cbor.encode(transactionData);
    console.log("🔥 CBOR for Initial State: ", cborData);

    // Encryption and sharding using Reed-Solomon
    const {shardedData, numShardsPerServer} = await encryptAndShardData(cborData, numServers, SRC);
    console.log("🔥 CBOR after Encryption and Sharding: ", shardedData);

    // HMAC calculation for each shard
    const hmacResults = await calculateHMACsForShards(shardedData, signKeys.slice(1));
    cborData = cbor.encode([...hmacResults, ...shardedData]);
    console.log("🔥 CBOR after HMAC Calculation: ", cborData);

    // Sending data to server
    const responseData = await sendToServer(endpoint, cborData);
    const decodedResponse = cbor.decode(new Uint8Array(responseData));
...
async function encryptAndShardData(data, numServers, SRC) {
  const shards = calculateReedSolomonShards(new Uint8Array(data), numServers);
  const numShardsPerServer = calculateNumberOfShardsPerServer(shards, numServers);
  let encryptedShards = [];

  for (let i = 0; i < shards.length; i++) {
    const keyIndex = Math.trunc(i / numShardsPerServer);
    // Corrected from encryptShard to encrypt
    const encryptedShard = await encrypt(shards[i], await importAesGcmKey(encryptKeys[keyIndex]), SRC);
    encryptedShards.push(encryptedShard);
  }

  return {shardedData: encryptedShards, numShardsPerServer};
}

async function calculateHMACsForShards(shards, signKeys) {
  const hmacResults = [];
  for (const [index, shard] of shards.entries()) {
    const hmac = await calculateHMAC(shard, await importHmacKey(signKeys[index % signKeys.length]));
    hmacResults.push(hmac.slice(0, 32)); 
  }
  return hmacResults;
}
...
async function sendToServer(endpoint, data) {
  try {
    const response = await axios.post(endpoint, new Uint8Array(data), {
      headers: {
        "Content-Type": "application/octet-stream",
        "Accept": "application/octet-stream"
      },
