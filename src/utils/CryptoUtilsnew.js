...
export async function exportCryptoKeyToJwk(key) {
    if (!(key instanceof CryptoKey)) {
        throw new Error("The key to be exported is not a valid CryptoKey object.");
    }
...
export async function generateTokenOrNonce() {
  const array = new Uint8Array(16); // For a 128-bit nonce
  window.crypto.getRandomValues(array);
  return array;
}

export async function importCryptoKeyFromJwk(jwk, algorithm, keyUsages) {
  return await window.crypto.subtle.importKey(
    'jwk',
    jwk,
    algorithm,
